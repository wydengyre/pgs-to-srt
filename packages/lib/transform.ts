...
import { SegmentTypes } from "./parse.js";
import { z } from "zod";
...
type Segment = { currentOffset: number; pts: number } & (
	| ({ type: typeof $pcs } & Pcs)
	| ({ type: typeof $pds } & Pds)
	| ({ type: typeof $ods } & Ods)
);
...
export async function* iterOds(
	...
	for await (const seg of segs) {
		const pds = seg.pds.get(seg.paletteId) as Uint32Array;
		const odss = seg.ods.values();
		for (const ods of odss) {
			yield { offset: seg.offset, startTime: seg.startTime, endTime: seg.endTime, ods, pds };
		}
	}
}
